...
ACME_HTTP_WEBROOT="/var/www/deploy-web-acme"
CADDY_MAIN_CONFIG="/etc/caddy/Caddyfile"
CADDY_SITES_DIR="/etc/caddy/conf.d"
...
save_state() {
...

  {
    ...
    printf 'WEB_CONFIG_PATH=%s\n' "$(quote_state_value "${WEB_CONFIG_PATH:-}")"
...
discover_cloudflare_zone_id() {
  ...
  while [[ "$candidate" == *.* ]]; do
    ...
    if grep -q '"success"[[:space:]]*:[[:space:]]*true' <<<"$response"; then
      if grep -q "\"name\"[[:space:]]*:[[:space:]]*\"${candidate}\"" <<<"$response"; then
        zone_id="$(printf '%s\n' "$response" | json_string_value "id")"
        zone_name="$candidate"
        CF_ZONE_ID="$zone_id"
...
track_config_file() {
  local path="$1"
  local backup="${path}.txn.${TIMESTAMP}"

  if [[ -e "$path" && ! -e "$backup" ]]; then
    cp -a "$path" "$backup"
    TXN_BACKUPS+=("${path}|${backup}|file")
  elif [[ ! -e "$path" ]]; then
    TXN_BACKUPS+=("${path}||missing")
  fi
}
...
rollback_config_transaction() {
  ...
  warn "配置失败，正在回滚已修改的配置文件..."
  for (( idx=${#TXN_BACKUPS[@]}-1; idx>=0; idx-- )); do
    item="${TXN_BACKUPS[$idx]}"
    IFS='|' read -r path backup kind <<<"$item"
    if [[ "$kind" == "file" && -n "$backup" && -e "$backup" ]]; then
      cp -a "$backup" "$path"
...
write_nginx_config() {
  ...
  WEB_CONFIG_PATH="$conf_path"

  track_config_file "$conf_path"
...
EOF
  ...
  if [[ -d /etc/nginx/sites-enabled ]]; then
    track_config_file "/etc/nginx/sites-enabled/${conf_name}.conf"
  ...
  if [[ -e /etc/nginx/sites-enabled/default ]]; then
    ...
    if [[ "$disable_default" == "y" ]]; then
      track_config_file /etc/nginx/sites-enabled/default
      mv /etc/nginx/sites-enabled/default "/etc/nginx/sites-enabled/default.bak.${TIMESTAMP}"
    fi
  fi

  nginx -t && enable_and_restart_service nginx
}
...
issue_nginx_certificate() {
  ...
  mkdir -p "$CERT_DIR" "$ACME_HTTP_WEBROOT"

  log "使用 acme.sh 通过 HTTP 验证为 ${domain} 申请证书..."
  "$acme_bin" --issue -w "$ACME_HTTP_WEBROOT" --server "$acme_server" "${ACME_DOMAIN_ARGS[@]}"

  reload_cmd="$(service_reload_command nginx)"
  log "安装证书到 ${CERT_DIR}，并配置续期后自动 reload nginx..."
...
server {
    ...
    location / {
        return 200 "ACME HTTP validation is ready.\\n";
...
grant_caddy_cert_access() {
  ...
  if [[ -n "$caddy_user" ]]; then
    ...
    chmod 755 "$cert_parent"
    chmod 750 "$CERT_DIR"
    chmod 640 "$CERT_FULLCHAIN" "$CERT_CERT" "$CERT_CA"
    chmod 640 "$CERT_KEY"
  else
...
ensure_caddy_import_config() {
  mkdir -p /etc/caddy "$CADDY_SITES_DIR"
  if [[ ! -f "${CADDY_SITES_DIR}/00-placeholder.caddy" ]]; then
    printf '# deploy-web placeholder\n' >"${CADDY_SITES_DIR}/00-placeholder.caddy"
  fi

  if [[ ! -f "$CADDY_MAIN_CONFIG" ]]; then
    track_config_file "$CADDY_MAIN_CONFIG"
...
EOF
  ...
  if ! grep -Fq "import ${CADDY_SITES_DIR}/*.caddy" "$CADDY_MAIN_CONFIG"; then
    track_config_file "$CADDY_MAIN_CONFIG"
    cat >>"$CADDY_MAIN_CONFIG" <<EOF
...
caddy_site_config_path() {
  local domain="$1"

  printf '%s/%s.caddy' "$CADDY_SITES_DIR" "$(safe_name_for_domain "$domain")"
}
...
write_caddy_config() {
  ...
  conf_path="$(caddy_site_config_path "$domain")"
  WEB_CONFIG_PATH="$conf_path"
...

  {
    ...
    case "$cert_provider" in
      ...
      zerossl)
        printf '            dir %s\n' "$ZEROSSL_ACME_SERVER"
        printf '            eab %s %s\n' "$zerossl_eab_kid" "$zerossl_eab_hmac"
        ;;
    esac
    if [[ -n "$email" ]]; then
      printf '            email %s\n' "$email"
  ...
  caddy validate --config "$CADDY_MAIN_CONFIG" && enable_and_restart_service caddy
}
...
deploy_site() {
  ...
  case "$RUNTIME" in
    nginx)
      install_nginx_stack || fail_and_rollback nginx "Nginx 安装失败。"
      if [[ "$CHALLENGE_MODE" == "dns-api" ]]; then
        issue_dns_certificate "$DOMAIN" "$EMAIL" "$CERT_PROVIDER" "$ZEROSSL_EAB_KID" "$ZEROSSL_EAB_HMAC" nginx || fail_and_rollback nginx "DNS API 证书申请失败。"
        write_nginx_config_with_cert "$DOMAIN" "$MODE" "$BACKEND" "$STATIC_ROOT" "$CERT_FULLCHAIN" "$CERT_KEY" || fail_and_rollback nginx "Nginx HTTPS 配置失败，已回滚。"
      else
        write_nginx_acme_http_config "$DOMAIN" || fail_and_rollback nginx "Nginx ACME HTTP 校验配置失败，已回滚。"
        issue_nginx_certificate "$DOMAIN" "$EMAIL" "$CERT_PROVIDER" "$ZEROSSL_EAB_KID" "$ZEROSSL_EAB_HMAC" || fail_and_rollback nginx "HTTP 证书申请失败，已回滚 Nginx 配置。"
        write_nginx_config_with_cert "$DOMAIN" "$MODE" "$BACKEND" "$STATIC_ROOT" "$CERT_FULLCHAIN" "$CERT_KEY" || fail_and_rollback nginx "Nginx HTTPS 配置失败，已回滚。"
      fi
      ;;
    caddy)
      install_caddy_stack || fail_and_rollback caddy "Caddy 安装失败。"
      if [[ "$CHALLENGE_MODE" == "dns-api" ]]; then
        issue_dns_certificate "$DOMAIN" "$EMAIL" "$CERT_PROVIDER" "$ZEROSSL_EAB_KID" "$ZEROSSL_EAB_HMAC" caddy || fail_and_rollback caddy "DNS API 证书申请失败。"
        write_caddy_config_with_cert "$DOMAIN" "$MODE" "$BACKEND" "$STATIC_ROOT" "$CERT_FULLCHAIN" "$CERT_KEY" || fail_and_rollback caddy "Caddy 站点配置失败，已回滚。"
      else
        write_caddy_config "$DOMAIN" "$EMAIL" "$CERT_PROVIDER" "$ZEROSSL_EAB_KID" "$ZEROSSL_EAB_HMAC" "$MODE" "$BACKEND" "$STATIC_ROOT" || fail_and_rollback caddy "Caddy 站点配置失败，已回滚。"
      fi
      ;;
  esac
  commit_config_transaction
...
delete_site() {
  ...
  case "${RUNTIME:-}" in
    nginx)
      track_config_file "/etc/nginx/sites-enabled/${conf_name}.conf"
      track_config_file "/etc/nginx/sites-available/${conf_name}.conf"
      track_config_file "/etc/nginx/conf.d/${conf_name}.conf"
      ...
      nginx -t && reload_service nginx || fail_and_rollback nginx "删除 Nginx 站点配置失败，已回滚。"
      ;;
    caddy)
      caddy_site_file="$(caddy_site_config_path "$DOMAIN")"
      track_config_file "$caddy_site_file"
      rm -f "$caddy_site_file"
      caddy validate --config "$CADDY_MAIN_CONFIG" && reload_service caddy || fail_and_rollback caddy "删除 Caddy 站点配置失败，已回滚。"
      ;;
    none)
